{$resource Resources/_Pen.png}
{$resource Resources/sPen.png}
{$resource Resources/Fill.png}
{$resource Resources/sFill.png}
{$resource Resources/Vd.png}
{$resource Resources/sVd.png}
{$resource Resources/Palitra.png}
{TODO:
+ Исправить перерисовку при масштабировании
+ Убрать режим отображения макета
+ Загружать по умолчанию отладочное изображение
+ Рисовать всё изображение, без обрезки
* Центровать изображение
* Исправить масштабирование - только +2x -2x ESX ESY EDX EDY...
* Перенести этот код на Python
- Исправить проблемы с рисованием при нестандартном масштабе 777 (???)
- Вернуть Undo
- Исправить отрисовку сетки
- Уменьшить количество вызовов Redraw
- Оптимизировать код
- Центровать отрисовку
- Вынести общий код в отдельный модуль
- Создать отдельную кнопку справки, не вызывать справку при запуске
- Динамическое обновление при выделении
- Перемещение картинки с зажатым роликом мыши
- Исправить невозможность установки выделения рядом с прошлой позицией
- Исправить ошибку Redraw
}
...
var
  ...
  //Colors
  cBack := RGB(230, 230, 230);
  cLight := RGB(250, 250, 250);
  cGray := RGB(128, 128, 128);
  cBlack := RGB(0, 0, 0);
...
begin
  GraphABC.Brush.Color := cGray;
  FillRoundRect(X, Y, X + _W, Y + _H, 4, 4);
  if (MouseX > X) and (MouseX < X + _W) and (MouseY > Y) and (MouseY < Y + _H) then
    GraphABC.Brush.Color := cLight
  else
    GraphABC.Brush.Color := cBack;
  FillRoundRect(X + 1, Y + 1, X + _W - 1, Y + _H - 1, 4, 4);
  GraphABC.Font.Color := cBlack;
...
var
  ...
  begin
    ...
    GraphABC.Brush.Color := cGray;
    FillRoundRect(TX - 3, TY - 3, TX + Width + 3, TY + Height + 3, 4, 4);
    GraphABC.Brush.Color := cBack;
    FillRoundRect(TX - 2, TY - 2, TX + Width + 2, TY + Height + 2, 4, 4);
...
begin
  if FileName <> '' then
  begin
    var ToLoad := Picture.Create(FileName);
    TW := ToLoad.Width;
    TH := ToLoad.Height;
...
begin
  ClearTile(clSilver);
  i := 0; while (i < TH) and (i * PixelSize < H) do
  begin
    j := 0; while (j < TW) and (j * PixelSize < W) do
    begin
      if (j > -1) and (j < TW) then
        if (i > -1) and (i < TH) then
        begin
          GraphABC.Brush.Color := Tile[j, i];
          if Tool = 2 then
            if (j >= VSX) and (j < VSX + VW) and (i >= VSY) and (i < VSY + VH) then
              GraphABC.Brush.Color := ARGB(Tile[j, i].A, Tile[j, i].R div 2 + 128, Tile[j, i].G div 2 + 64, Tile[j, i].B div 2 + 64);
...
procedure DrawBuffer_;
var
  i, j: integer;
begin
  i := 0; while (i < VH) and (i * PixelSize < H) do
  begin
    j := 0; while (j < VW) and (j * PixelSize < W) do
    begin
      begin
        GraphABC.Brush.Color := Buffer[j, i];
        if Tool = 2 then
    ...
    begin
      ...
      begin
        ...
        if ToSee[i].X + 1 < TW then
          ...
          begin
            SetLength(TmSee, Length(TmSee) + 1);
...
begin
  ...
  if MousePressed then
  begin
    MX := Round((MouseX - PixelSize / 2) / PixelSize);
    MY := Round((MouseY - PixelSize / 2) / PixelSize);
    if MX < TW then
      if MX >= 0 then
        if MY < TH then
          if MY >= 0 then
            //if ShowMaket then
            if MouseCode = 0 then begin
              Swap(FirstColor, TwoColor); Pause := true;
              if RGBtoHSV(FirstColor.R, FirstColor.G, FirstColor.B).H <> 0 then
                HUE := RGBtoHSV(FirstColor.R, FirstColor.G, FirstColor.B).H;
              Sat := RGBtoHSV(FirstColor.R, FirstColor.G, FirstColor.B).S;
              Ve := RGBtoHSV(FirstColor.R, FirstColor.G, FirstColor.B).V; end
              else
            if MouseCode = 2 then begin
              FirstColor := Tile[MX, MY];
              if RGBtoHSV(FirstColor.R, FirstColor.G, FirstColor.B).H <> 0 then
                HUE := RGBtoHSV(FirstColor.R, FirstColor.G, FirstColor.B).H;
              Sat := RGBtoHSV(FirstColor.R, FirstColor.G, FirstColor.B).S;
              Ve := RGBtoHSV(FirstColor.R, FirstColor.G, FirstColor.B).V; end
            ...
            begin
              case Tool of
                0:
                  begin
                    ...
                    begin
                      MX := Round((MouseX - PixelSize / 2) / PixelSize);
                      MY := Round((MouseY - PixelSize / 2) / PixelSize);
                      if (MX >= 0) and (MY >= 0) and (MX < TW) and (MY < TH) and
                                                       (MX < Length(Tile, 0)) and (MY < Length(Tile, 1)) then
                      begin
                        GraphABC.Brush.Color := FirstColor;
                        FillRect(Round(PixelSize * (MX)),
                                   Round(PixelSize * (MY)),
                                   Round(PixelSize * ((MX + 1))) - MP,
                                   Round(PixelSize * ((MY + 1))) - MP);
                        if FirstColor.A > 0 then Tile[MX, MY] := GetPixel(Round(PixelSize * (MX)) + 1, Round(PixelSize * (MY)) + 1) else
                ...
                2:
                  begin
                    if (MX < VSX) or (MY < VSY) or (MX > VSX + VW) or (MY > VSY + VH) then
                    begin
                      VSX := MX;
                      VSY := MY;
                      while MousePressed do;
                      if (Abs(MX - Round((MouseX - PixelSize / 2) / PixelSize)) > 1) and
                           (Abs(MY - Round((MouseY - PixelSize / 2) / PixelSize)) > 1) then
                      begin
                        VW := Round((MouseX - PixelSize / 2) / PixelSize) - MX + 1;
                        VH := Round((MouseY - PixelSize / 2) / PixelSize) - MY + 1;
                        if VW < 0 then begin VSX += VW - 1; VW := -VW + 2 end;
                        if VH < 0 then begin VSY += VH - 1; VH := -VH + 2 end;
                        if VSX < 0 then VSX := 0;
                        if VSY < 0 then VSY := 0;
                        if VSX + VW > TW then VW := VSX;
                        if VSY + VH > TH then VH := VSY;
                    ...
                    begin
                      ...
                      while MousePressed do
                      begin
                        VSX := Round((MouseX - PixelSize / 2) / PixelSize) - MsX;
                        VSY := Round((MouseY - PixelSize / 2) / PixelSize) - MsY;
                        ClearTile(clGray);
                        DrawTile;
                        //DrawBuffer;
                        Redraw;
  ...
  begin
    case KeyCode of
      ...
      VK_L:
        begin
          ...
          tmp.ShowDialog;
          MainDir := DelName(tmp.FileName);
          LoadPicture(tmp.FileName);
          DrawTile; ToolsPanel; Redraw;
          Pause := true;
        end;
    ...
    Pause := false;
    if notDraw then DrawTile; ToolsPanel; Redraw;
  end;
end;
...
begin
  ...
  ToolPics[0] := Picture.Create('Resources/_Pen.png');
  ToolPics[1] := Picture.Create('Resources/Fill.png');
  ToolPics[2] := Picture.Create('Resources/Vd.png');
  ToolPics[3] := Picture.Create('Resources/sPen.png');
  ToolPics[4] := Picture.Create('Resources/sFill.png');
  ToolPics[5] := Picture.Create('Resources/sVD.png');
  SetLength(TileSet, 8, 8);
  for i: integer := 0 to 7 do
    for j: integer := 0 to 7 do
      TileSet[j, i] := Picture.Create(1, 1);//Чтобы не было ошибок при обращении к несозданному тайлу
  
  var ToLoad := Picture.Create('Resources/Palitra.png');
  for i: integer := 0 to 7 do
    for j: integer := 0 to 7 do
      Palitra[j, i] := ToLoad.GetPixel(j, i);
  LoadPicture('Картинки/debug.png');
  DrawTile;
  Redraw;
end;
